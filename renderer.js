    ...
    for (numeroEpreuveSouhaitee of epreuvesSouhaitees) {
        ...
        if (!isNaN(parseInt(placesRestantes))
            ...
            ouvrirFenetreAvertissement(`Place disponible pour l'épreuve ${numeroEpreuveSouhaitee}`);
             ...
             // Permet d'envoyer un 'event' avec un contenu au `main.js` afin de déclencher des actions par le processus principal 
            ipcRenderer.send("place-disponible", url, concours, numeroEpreuveSouhaitee, navigateurActivee, notificationActivee, envoiMailActive);
